...
function addTodo() {
  ...
  todos.unshift({ text: displayText, completed: false });
  saveTodos();
...
function exportToExcel() {
  ...
  // CSV 헤더
  const headers = ['순서', '날짜', '담당수의사', '개체번호'];
...
function renderSortedTodos() {
  ...
  // 날짜순으로 정렬하고, 같은 날짜 내에서는 개체번호 순으로 정렬
  const sortedTodos = todos.sort((a, b) => {
    const partsA = a.text.split(', ');
    const partsB = b.text.split(', ');
    const dateA = partsA[0] || '';
    const dateB = partsB[0] || '';
    const animalA = partsA[2] || '';
    const animalB = partsB[2] || '';
    ...
    if (datePartsA.length >= 3 && datePartsB.length >= 3) {
      ...
      if (dateObjA.getTime() !== dateObjB.getTime()) {
        return dateObjA - dateObjB; // 날짜가 다르면 날짜순 정렬
      }
    }
    
    // 날짜가 같으면 개체번호 순으로 정렬
    const numA = parseInt(animalA) || 0;
    const numB = parseInt(animalB) || 0;
    return numA - numB;
  });
